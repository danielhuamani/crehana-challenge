...
from src.tasks.schemas import TaskCreateBody, TaskResponse
...
def test_read_tasks(client: TestClient, db: Session) -> None:
...
def test_updated_task(client: TestClient, db: Session) -> None:
    task_body = TaskCreateBody(title="prueba test", description="description test")
    repository = TaskRepository(session=db)
    task_response: TaskResponse = TaskService(repository=repository).create(task_body)
    data = {
        "title": "Prueba 123",
        "description": "Description prueba 123",
        "completed": True,
    }
    response = client.put(f"/tasks/{task_response.id}", json=data)
    response_json = response.json()
    assert response.status_code == status.HTTP_200_OK
    assert response_json["title"] == data["title"]
    assert response_json["description"] == data["description"]
...
def test_read_task_not_found(client: TestClient, db: Session) -> None:
    task_body = TaskCreateBody(title="prueba test", description="description test")
    repository = TaskRepository(session=db)
    TaskService(repository=repository).create(task_body)
